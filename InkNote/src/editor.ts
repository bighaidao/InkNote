...
/** 任务项：兼容标准无序/有序形式，也兼容单独 `[x] 文本` 的简写。 */
...
function buildPreviewSets(
  ...
  tree.iterate({
    enter(node) {
      ...
      if (n === "ListItem") {
        ...
        const indentStyle =
          indentExtra > 0
            ? `--md-list-marker-left: calc(var(--editor-padding-x) + ${indentExtra}px); padding-left: calc(var(--editor-padding-x) + var(--md-list-marker-width, var(--editor-list-marker-offset)) + ${indentExtra}px);`
            : "";
        ...
        if (ordered) {
          ...
          if (safe) {
            // 1.、10.、100. 不能共用固定宽度，否则两位数起会挤出正文左缘。
            const markerWidth = Math.max(1.35, safe.length * 0.58 + 0.3).toFixed(2);
            markerStyle = `--md-marker: "${safe}"; --md-list-marker-width: ${markerWidth}em;`;
          }
        }
        ...
        const continuationStyle = `${indentStyle}${ordered ? markerStyle : ""}`;
        for (let i = firstLine.number + 1; i <= lastLine.number; i++) {
          addLineClass(
            doc.line(i).from,
            "md-list-item",
            continuationStyle ? { style: continuationStyle } : undefined,
          );
        }
        return;
      }
  ...
  // `[x] 文本` 不是标准 GFM ListItem，Lezer 不会生成 Task 节点；作为编辑器
  // 兼容语法单独补装饰，保留源文件原样并复用同一套点击切换逻辑。
  for (let number = 1; number <= doc.lines; number++) {
    const line = doc.line(number);
    const match = TASK_LINE_RE.exec(line.text);
    if (!match || match[1].trim() || inCode(line.from, line.to)) continue;
    const markFrom = line.from + match[1].length;
    const markTo = markFrom + 3 + (match[3] ? 1 : 0);
    hide(markFrom, markTo, NEVER_REVEAL);
    addLineClass(
      line.from,
      match[2].toLowerCase() === "x" ? "md-task-item md-task-done" : "md-task-item",
    );
  }
...
/** 标题行首回车：停在新段落里，而不是插入后仍跟着标题导致空行立即折叠。 */
function insertParagraphBeforeHeading(view: EditorView): boolean {
  const sel = view.state.selection.main;
  if (!sel.empty) return false;
  const line = view.state.doc.lineAt(sel.head);
  if (sel.head !== line.from || !/^\s{0,3}#{1,6}(?:\s|$)/.test(line.text)) return false;
...
function mediaHandlers(onOpenMarkdown?: (content: string, path?: string) => void, documentId = ""): Extension {
  return EditorView.domEventHandlers({
    ...
    mousedown(event, view) {
        ...
        if (url) {
          ...
          if (url.startsWith("#")) {
            ...
            try {
              ...
              /* keep the literal fragment when a document contains malformed escaping */
            }
      ...
      // 点击伪元素方框时 posAtCoords 偶尔会落到相邻行；实际命中的行 DOM 才是准绳。
      const taskLinePos = view.posAtDOM(taskLine, 0);
      const line = view.state.doc.lineAt(taskLinePos);
      const m = TASK_LINE_RE.exec(line.text);
      if (!m) return false;

      const bracketPos = line.from + m[1].length;
      const onBrackets = pos != null && pos >= bracketPos && pos <= bracketPos + 3;

      const rect = taskLine.getBoundingClientRect();
      const style = getComputedStyle(taskLine);
      const fontSize = Number.parseFloat(style.fontSize) || 16;
      const paddingLeft = Number.parseFloat(style.paddingLeft);
      const nested = taskLine.getAttribute("style")?.includes("--md-list-marker-left") ?? false;
      const markerOffsetRaw = (
        nested ? style.getPropertyValue("--md-list-marker-width") : ""
      ).trim() || style.getPropertyValue("--editor-list-marker-offset").trim();
      const markerOffsetValue = Number.parseFloat(markerOffsetRaw);
      const markerOffset = Number.isFinite(markerOffsetValue)
        ? markerOffsetValue * (markerOffsetRaw.endsWith("em") ? fontSize : 1)
        : 22;
      const checkboxLeft = Number.isFinite(paddingLeft)
        ? rect.left + paddingLeft - markerOffset
        : rect.left;
      // 视觉方框 14px，四周再留出容易点中的热区；嵌套项不能按整行左侧判断。
      const onCheckboxVisual =
        event.clientX >= checkboxLeft - 6 && event.clientX <= checkboxLeft + 20;
...
export function createEditor(
  ...
  return {
    ...
    refreshPreview: () => {
      if (mode === "preview") view.dispatch({ effects: rebuildPreviewEffect.of() });
    },
