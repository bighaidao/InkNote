...
async function copyHtmlFromView(view: EditorView) {
  ...
  try {
    const { markdownToBodyHtml } = await import("../../render/export");
...
export function runEditorAction(view: EditorView, action: EditorAction): boolean {
  switch (action) {
    ...
    case "pastePlain":
      return clipboardAction(view, "pastePlain");
    case "selectAll":
      return selectAll(view);
    case "find":
      editorRequestSearch(false);
      return true;
    case "findReplace":
      editorRequestSearch(true);
      return true;
    case "bold":
    ...
    case "codeBlock":
      return insertBlock(view, "```\n\n```", -4);
    case "toc": {
      const toc = buildMarkdownToc(view.state.doc.toString());
      if (!toc) {
        editorShowMessage(t(getLocale(), "toast.noHeadings"));
        return true;
      }
      return insertBlock(view, toc);
    }
    case "table":
...
export function editorActionKeymap(): Array<{ key: string; run: (view: EditorView) => boolean }> {
  return [
    ...
    { key: "Mod-Shift-v", run: (v) => runEditorAction(v, "pastePlain") },
