...
const CELL_ALIGN_RE = /^<!--inknote-align:(left|center|right)-->\s*/;

function parseCellValue(value: string): { content: string; align?: TableAlign } {
  const match = CELL_ALIGN_RE.exec(value);
  return match
    ? { content: value.slice(match[0].length), align: match[1] as TableAlign }
    : { content: value };
}

function serializedCellValue(cell: HTMLElement): string {
  const content = cellRawText(cell);
  const align = cell.dataset.cellAlign as TableAlign | undefined;
  return align ? `<!--inknote-align:${align}-->${content}` : content;
}
...
export function renderInlineMarkdown(src: string): string {
  const parts = escapeHtml(parseCellValue(src).content).split(/(`[^`]+`)/g);
  return parts
    .map((part, i) => {
...
function setCellRaw(cell: HTMLElement, raw: string, columnAlign: TableAlign = "left") {
  const parsed = parseCellValue(raw);
  cell.dataset.raw = parsed.content;
  if (parsed.align) cell.dataset.cellAlign = parsed.align;
  else delete cell.dataset.cellAlign;
  cell.style.textAlign = cellAlignStyle(parsed.align ?? columnAlign);
  if (document.activeElement === cell) {
    if (cell.textContent !== parsed.content) cell.textContent = parsed.content;
  } else {
    cell.innerHTML = renderInlineMarkdown(parsed.content);
  }
}
...
function readTableFromDom(wrap: HTMLElement): TableData {
  ...
  wrap.querySelectorAll<HTMLElement>("thead th").forEach((th) => {
    header.push(serializedCellValue(th));
  });
  ...
  wrap.querySelectorAll<HTMLElement>("tbody tr").forEach((tr) => {
    ...
    tr.querySelectorAll<HTMLElement>("td").forEach((td) => {
      row.push(serializedCellValue(td));
    });
...
/** 对齐：未框选时作用于整表；框选后只写入所选单元格的轻量 Markdown 注释。 */
function applyAlign(wrap: HTMLElement, view: EditorView, align: TableAlign) {
  const cells = selectedCells(wrap);
  if (cells.length) {
    for (const cell of cells) {
      cell.dataset.cellAlign = align;
      cell.style.textAlign = cellAlignStyle(align);
    }
    flushSync(wrap, view);
    return;
  }
...
function appendEditableRow(
  ...
  cells.forEach((cell, i) => {
    ...
    setCellRaw(el, cell, aligns[i] ?? "left");
    tr.appendChild(el);
  });
}
...
export class TableWidget extends WidgetType {
  ...
  updateDOM(dom: HTMLElement) {
    ...
    this.header.forEach((text, i) => {
      const th = ths[i];
      if (!th) return;
      // 对齐只改样式，不会破坏 contenteditable 光标；聚焦单元格也要立即更新。
      th.style.textAlign = cellAlignStyle((th.dataset.cellAlign as TableAlign | undefined) ?? aligns[i] ?? "left");
      // 正在输入的单元格不能重写内容，否则光标会被打掉
      if (th === activeEl) return;
      setCellRaw(th, text, aligns[i] ?? "left");
    });
    ...
    this.rows.forEach((row, ri) => {
      ...
      row.forEach((text, ci) => {
        const td = tds[ci];
        if (!td) return;
        td.style.textAlign = cellAlignStyle((td.dataset.cellAlign as TableAlign | undefined) ?? aligns[ci] ?? "left");
        if (td === activeEl) return;
        setCellRaw(td, text, aligns[ci] ?? "left");
      });
    });
