...
- 메뉴 또는 폴더에서 새 창을 열 수 있습니다. 각 창은 **독립된 작업 공간**으로 자체 탭, 사이드바, 파일 트리를 갖습니다.
- 창별 파일/디렉터리 감시가 각 작업 공간을 동기화하고, 창 수와 각 창의 작업 공간은 **재시작 시 복원**됩니다(최대 5개).
- **macOS**: 마지막 창을 닫아도 Dock에 유지되며, Dock에서 다시 열 수 있고 네이티브 창 탭을 지원합니다.

### 환영 화면 재설계

- Obsidian 스타일의 글라스 모피즘 환영 패널에 **빠른 작업 카드**(새로 만들기 / 열기 / 폴더 열기 / 샘플)와 **최근 문서 카드** 목록(빈 상태 포함)을 추가했습니다.
