...
- メニューやフォルダーから新しいウィンドウを開けます。各ウィンドウは**独立したワークスペース**で、独自のタブ・サイドバー・ファイルツリーを持ちます。
- ウィンドウごとのファイル/ディレクトリ監視で各ワークスペースを同期し、ウィンドウ数と各ワークスペースは**再起動時に復元**されます（上限 5）。
- **macOS**：最後のウィンドウを閉じても Dock に常駐し、Dock から再オープンでき、ネイティブなウィンドウタブに対応します。

### ようこそ画面の再デザイン

- Obsidian 風のガラスモーフィズムのウェルカムパネルに**クイックアクションカード**（新規 / 開く / フォルダーを開く / サンプル）と**最近のドキュメントカード**（空状態含む）を追加しました。
