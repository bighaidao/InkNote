...
- 從選單或資料夾開啟新視窗——每個視窗都是**獨立工作區**，擁有自己的標籤頁、側邊欄與檔案樹。
- 每個視窗獨立的檔案/目錄監聽器保持各工作區同步；視窗數量與各視窗工作區在**重新啟動後還原**（上限 5）。
- **macOS**：關閉最後一個視窗後 App 仍留在 Dock，可從 Dock 重新開啟，並支援系統原生視窗標籤。

### 歡迎頁改版

- 採用 Obsidian 風格玻璃擬態設計的歡迎面板，提供**快速操作卡片**（新增 / 開啟 / 開啟資料夾 / 範例）與**最近文件卡片流**，並包含空狀態。
...
<picture>
  <img alt="InkNote Star 成長曲線" src="docs/assets/star-history.svg" />
</picture>
